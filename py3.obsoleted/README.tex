%%% -*- Mode: LaTeX; -*-
%       RCS: $Id: README.tex,v 1.3 2018-11-07 23:00:42 lsipusr Exp $
...
\title{roPerf.bearerTokenExample}
...
\begin{itemize}
\item In documentation, refer to plpc-180056 (mmwsIcm BCP) and plpc-180057 (Testing Framework)
\item Convert ./bin/roPerf-bearerTokenExample.sh to a python ICM aware of pkgBases 
\item Get Port number from ./roPerf/bearerTokenExample-config/svcInfo/portNu
\item Get  ./roPerf/bearerTokenExample-config/pkgInfo/fp/icmsPkgVarBaseDir/ and build and run in there
\item Reflect the recuirements.txt of generated code in ./setup.py dependncies. so that it is not needed.
\item Deal with automation of java swagger code generator -- perhaps a dummy pkg, ...
\item Understand BCP for flask productization
\item Make this be a starting point for other services
\item Attach the controller to mmwsIcm.bearer token.
\item Create a scenario tester for bearerTokenExample.
\item Test pure out of the box ``just runs'' experience.
\item Start transitioning this example to become a wsIcm. unisos.icm needs to become py3 first.
\end{itemize}
